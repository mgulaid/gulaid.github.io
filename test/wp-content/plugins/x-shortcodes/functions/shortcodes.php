...
// function x_shortcode_button_group( $atts, $content = null ) {
//   extract( shortcode_atts( array(
//     'id'    => '',
//     'class' => '',
//     'style' => '',
...
//   $id    = ( $id    != ''         ) ? 'id="' . esc_attr( $id ) . '"' : '';
//   $class = ( $class != ''         ) ? 'x-btn-toolbar x-btn-group ' . esc_attr( $class ) : 'x-btn-toolbar x-btn-group';
//   $style = ( $style != ''         ) ? 'style="' . $style . '"' : '';
//   $type  = ( $type  == 'vertical' ) ? ' x-btn-group-vertical' : '';
    
//   $output = "<div {$id} class=\"{$class}{$type}\" {$style}>" . do_shortcode( $content ) . "</div>";
  
//   return $output;
// }

// add_shortcode( 'button_group', 'x_shortcode_button_group' );
...
function x_shortcode_accordion_item( $atts, $content = null ) { // 2
  ...
  if ( $open == 'collapse in' ) {
    ...
    $output = "<div {$id} class=\"{$class}\" {$style}>"
              ...
              . '</div> <!-- end .x-accordion-heading -->'
              . "<div id=\"collapse-{$count}\" class=\"accordion-body {$open}\">"
                . '<div class="x-accordion-inner">'
                  . do_shortcode( $content )
                . '</div> <!-- end .x-accordion-inner -->'
              . '</div> <!-- end .x-accordion-body -->'
            . "</div> <!-- end .x-accordion-group -->";
...
function x_shortcode_post_author( $atts ) {
  ...
  $facebook_output   = ( $facebook )   ? "<a href=\"{$facebook}\" class=\"x-author-social\" title=\"Visit the Facebook Profile for {$display_name}\" target=\"_blank\"><i class=\"x-social-facebook\"></i> Facebook</a>" : '';
  $twitter_output    = ( $twitter )    ? "<a href=\"{$twitter}\" class=\"x-author-social\" title=\"Visit the Twitter Profile for {$display_name}\" target=\"_blank\"><i class=\"x-social-twitter\"></i> Twitter</a>" : '';
  $googleplus_output = ( $googleplus ) ? "<a href=\"{$googleplus}\" class=\"x-author-social\" title=\"Visit the Google+ Profile for {$display_name}\" target=\"_blank\"><i class=\"x-social-google-plus\"></i> Google+</a>" : '';
...
function x_shortcode_responsive_text( $atts ) {
  ...
  if ( $min_size == '' && $max_size == '' ) {
    $output = '<script>'
              . "jQuery('{$selector}').fitText({$compression});"
            . '</script>';    
  } else if ( $min_size != '' && $max_size == '' ) {
    $output = '<script>'
              . "jQuery('{$selector}').fitText({$compression}, { minFontSize: '{$min_size}' });"
            . '</script>';
  } else if ( $min_size == '' && $max_size != '' ) {
    $output = '<script>'
              . "jQuery('{$selector}').fitText({$compression}, { maxFontSize: '{$max_size}' });"
            . '</script>';
  } else if ( $min_size != '' && $max_size != '' ) {
    $output = '<script>'
              . "jQuery('{$selector}').fitText({$compression}, { minFontSize: '{$min_size}', maxFontSize: '{$max_size}' });"
            . '</script>';
  }
